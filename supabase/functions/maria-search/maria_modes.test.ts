...
Deno.test("MarIA - Intent Routing Validation", async () => {
  ...
  for (const scenario of scenarios) {
    const reply = await callAI(LOVABLE_API_KEY, "google/gemini-2.0-flash-lite-preview-02-05", PROMPTS.ROUTER, [{ role: "user", content: scenario.input }], 0);
...
Deno.test("MarIA - Consultivo Mode: Tone and Negative Check", async () => {
  ...
  const reply = await callAI(LOVABLE_API_KEY, "google/gemini-2.0-flash-lite-preview-02-05", PROMPTS.CONSULTIVO_CHAT, [{ role: "user", content: input }], 0);
...
Deno.test("MarIA - Busca Mode: Objective Check", async () => {
  ...
  const reply = await callAI(LOVABLE_API_KEY, "google/gemini-2.0-flash-lite-preview-02-05", PROMPTS.BUSCA_CHAT, [{ role: "user", content: input }], 0);
...
Deno.test("MarIA - Proprietario Mode: Logic Check", async () => {
  ...
  const reply = await callAI(LOVABLE_API_KEY, "google/gemini-2.0-flash-lite-preview-02-05", PROMPTS.PROPRIETARIO_CHAT, [{ role: "user", content: input }], 0);
...
Deno.test("MarIA - Comum Mode: Role Explanation", async () => {
  ...
  const reply = await callAI(LOVABLE_API_KEY, "google/gemini-2.0-flash-lite-preview-02-05", PROMPTS.COMUM_CHAT, [{ role: "user", content: input }], 0);
